                    ...
                    script {
						withEnv(["DOCKER_HOST=tcp://localhost:2375"]) {
							withCredentials([usernamePassword(credentialsId: 'harborLogin', usernameVariable: 'HARBOR_USERNAME', passwordVariable: 'HARBOR_PASSWORD')]) {
								sh """
									docker build -t ${imageName} -f src/ProductService.HttpApi.Host/Dockerfile --build-arg BUILD_CONFIGURATION=${buildEnv} .
									docker build -t ${latestName} -f src/ProductService.HttpApi.Host/Dockerfile --build-arg BUILD_CONFIGURATION=${buildEnv} .
